calculateItemsPerformace <- function(itemNames, dataset, naLevels = c("7", "8"), cntVal = "CNT", weightVal = "W_FSTUWT") {
    ...
    for (i in seq_along(itemNames)) {
        item <- dataset[,grep(colnames(dataset), pattern=substr(itemNames[i],1,7))]
        ...
        perfromance <- prop.table(unclass(by(dataset[,weightVal], list(dataset[,cntVal],  maxAns), base::sum, na.rm=TRUE)), 1)
        
        res[i,] <-  round(perfromance[,2] * 100,1)
    }
    res
}
